...
Page({
    data: {
        shop: null,
        status_arr: [ "", "待付款", "已付款", "待收货", "已完成", "已评价", "已关闭", "已退款" ],
        status_arr_group: [ "", "待付款", "待成团", "已成团，待发货", "已发货，待收货", "已完成，待评价", "已评价", "已取消", "已关闭", "已退款" ],
        tags: [ "全部", "待付款", "已付款", "待收货", "已完成" ],
        list: null,
        page: 1,
        size: 10,
        selected: 0,
    ...
    pay: function(t) {
        var a = t.currentTarget.id, s = this.data.list[a].orderNo;
        r.getPayJson({
            ordersNo: s
        }, function(t) {
            if (null != t.payJson) {
                ...
                wx.requestPayment({
                    ...
                    success: function(t) {
                        wx.showToast({
                            ...
                            complete: function() {
                                wx.navigateTo({
                                    url: "../orderdetail/orderdetail?orderNo=" + s
                                });
                            }
                        });
                    },
    ...
    cancel: function(a) {
        var s = a.currentTarget.id, o = this.data.list[s].orderNo;
        e.confirm("确认取消该订单？", "", function() {
            r.deleteOrders(o, function(a) {
                e.toast("订单已取消", function() {
                    t.data.list[s].status = 6, t.setData({
                        list: t.data.list
                    });
                });
            });
    ...
    receive: function(a) {
        ...
        e.confirm("确认商品已送达？", "", function() {
            r.receiptOrders(o, function(a) {
                t.data.list[s].status = 4, t.setData({
                    list: t.data.list
                });
            });
        });
